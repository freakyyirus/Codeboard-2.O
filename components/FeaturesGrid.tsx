            ...
            <div className="max-w-6xl mx-auto px-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    {features.map((feature, index) => (
                        <article
                            key={index}
                            className="group relative rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-6 transition-all duration-150 hover:-translate-y-0.5 hover:shadow-lg hover:border-[var(--elevated)]"
                            style={{ borderLeft: `3px solid ${feature.accent}` }}
                        >
                            <div className="flex flex-col h-full">
                                <div className="w-10 h-10 rounded-[6px] border border-[var(--border)] flex items-center justify-center text-[var(--text-secondary)] mb-5 bg-[var(--background)] group-hover:text-[var(--foreground)] transition-colors duration-150">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <article
                        className="rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-8 transition-all duration-150 hover:-translate-y-0.5 hover:shadow-lg hover:border-[var(--elevated)] group"
                    ...
                    >
                        <div className="mb-8 max-w-sm">
                            ...
                            <button className="mt-4 text-[var(--primary)] font-medium text-xs font-mono flex items-center gap-2 hover:gap-3 transition-all duration-150 uppercase tracking-wider">
                                Explore Analytics <ArrowRight className="w-3 h-3" />
                            </button>
                        </div>
                        {/* Chart mockup */}
                        <div className="w-full h-32 flex items-end justify-between gap-1.5 px-4 opacity-40 group-hover:opacity-80 transition-opacity duration-150">
                    ...
                    <article
                        className="rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-8 flex flex-col justify-center transition-all duration-150 hover:-translate-y-0.5 hover:shadow-lg hover:border-[var(--elevated)] group"
                        style={{ borderLeft: "3px solid var(--warning)" }}
                    >
                        <div className="text-center">
                            <div className="inline-flex items-center justify-center p-3 rounded-[6px] bg-[var(--background)] border border-[var(--border)] mb-4 text-[var(--text-secondary)] group-hover:text-[var(--foreground)] transition-colors duration-150">
                            ...
                            {/* Avatar stack */}
                            <div className="flex justify-center -space-x-2 opacity-40 group-hover:opacity-80 transition-opacity duration-150">
                                {[1, 2, 3].map((i) => (
